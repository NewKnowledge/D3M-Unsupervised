...
cd ../pipeline_runs
Datasets=('SEMI_1040_sylva_prior_MIN_METADATA' 'SEMI_1044_eye_movements_MIN_METADATA')
for i in "${Datasets[@]}"; do

  start=`date +%s`
  python3 -m d3m runtime -d /datasets/ fit-produce -p ../pipelines/*.json -i /datasets/seed_datasets_current/$i/TRAIN/dataset_TRAIN/datasetDoc.json -t /datasets/seed_datasets_current/$i/TEST/dataset_TEST/datasetDoc.json -r /datasets/seed_datasets_current/$i/${i}_problem/problemDoc.json -O ${i}.yml
  end=`date +%s`
  runtime=$((end-start))
